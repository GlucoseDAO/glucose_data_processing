...
class JaebDatabaseConverter(MonoUserDatabaseConverter):
    ...
    def _apply_database_specific_processing(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        if dropped.height > 0:
            summary = ", ".join(f"{r['len']:,} {r['event_type']}" for r in dropped.iter_rows(named=True))
            logger.info(f"  User {df['user_id'][0]}: dropped non-CGM rows ({summary})")
        return df.filter(pl.col("event_type") == JAEB_TRACE_EVENT_TYPE).with_columns(
